...
class Game {
    ...
    /**
     * @return if no check <0,any> , if single checker <1,checker_position>, if double checker <2,any>
     */
    std::pair<int,std::pair<int,int>> king_checked_move(const Move move,bool in_state=false);
    
    bool in_range(const std::pair<int,int>& position) const;
    /** checks wheter any piece of current player can make any valid move */
    bool any_possible_move();
    /** returns wheter position is reachable by current players pieces */
    bool is_reachable(const std::pair<int,int>& position);
